...
int main()
{
	...
	SceneObjectFactory::AddSphere(scene, 6, 3, 0, 1.5f, glm::vec3(1.0f, 0.0f, 0.0f));
	...
	/* Render. */
	c.Render(scene, 3000); // Keep number of rays N so that you can write N as N = X^2 for some integer X.
	c.WriteImageToTGA();
